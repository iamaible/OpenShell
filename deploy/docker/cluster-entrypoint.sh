...
# ---------------------------------------------------------------------------
# Origin note (Aible fork)
#
# This file was extracted verbatim from the published image
# `ghcr.io/nvidia/openshell/cluster:latest` (digest pinned in
# deploy/docker/Dockerfile.cluster) rather than copied from upstream `main`.
#
# Why: NVIDIA's CI builds `:latest` from an internal branch ahead of public
# `main`. The version of this script on `upstream/main` is older and does
# NOT substitute the `__IMAGE_PULL_POLICY__`, `__SANDBOX_IMAGE_PULL_POLICY__`,
# `__DB_URL__`, `__SSH_GATEWAY_HOST__`, `__SSH_GATEWAY_PORT__`,
# `__HOST_GATEWAY_IP__`, `__DISABLE_GATEWAY_AUTH__`, or `__DISABLE_TLS__`
# placeholders that our deploy/kube/manifests/openshell-helmchart.yaml relies
# on. Using the older `main` version would leave those literal strings in
# the HelmChart values and Helm install would fail.
#
# When upstream `main` catches up (or our fork lands a newer published image),
# diff this file against the upstream copy and resync. See
# deploy/docker/UPSTREAM-BASE-UPGRADE.md for the bump procedure.
# ---------------------------------------------------------------------------
