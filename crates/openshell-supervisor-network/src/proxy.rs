...
impl ProxyHandle {
    ...
    pub(crate) async fn start_with_bind_addr(
        ...
        // Clone before the accept-loop closure takes ownership so the direct
        // inference TLS listener below can also reference them.
        ...
        let inference_join = if let (Some(tls), Some(inf), Some(addr)) = (
            ...
            match TcpListener::bind(tls_addr).await {
                Ok(inference_listener) => {
                    {
                        let event = NetworkActivityBuilder::new(openshell_ocsf::ctx::ctx())
                            ...
                            .message(format!("Inference TLS listener on {tls_addr}"))
                            .build();
                        ocsf_emit!(event);
                    }
                    let tls_c = tls.clone();
                    ...
                    Some(tokio::spawn(async move {
                        loop {
                            match inference_listener.accept().await {
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        .await
                                        {
                                            Ok(InferenceOutcome::Denied { reason }) => {
                                                let event = NetworkActivityBuilder::new(
                                ...
                                Err(err) => {
                                    warn!("Inference TLS listener accept error: {err}");
                                    break;
                                }
                            }
                        }
                ...
                Err(e) => {
                    warn!("Failed to bind inference TLS listener on {tls_addr}: {e}");
                    None
                }
            }
        } else {
