...
///
/// This injects:
///   1. A volume referencing the `workspace` PVC.
///   2. A volume mount on the agent container at `/sandbox`.
///   3. An init container (same image) that seeds the PVC with the image's
...
fn apply_workspace_persistence(pod_template: &mut serde_json::Value, image: &str) {
    ...
    // 1. Add the PVC volume to spec.volumes
    let volumes = spec
        .entry("volumes")
        .or_insert_with(|| serde_json::json!([]))
        .as_array_mut();
    if let Some(volumes) = volumes {
        volumes.push(serde_json::json!({
            ...
            "persistentVolumeClaim": {
                "claimName": WORKSPACE_VOLUME_NAME
            }
        }));
    }

    // 2. Add workspace volume mount to the agent container
...
mod tests {
    ...
    fn workspace_persistence_injects_init_container_volume_and_mount() {
        ...
        assert_eq!(
            pvc_vol["persistentVolumeClaim"]["claimName"],
